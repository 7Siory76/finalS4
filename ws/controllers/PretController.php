...
class PretController
{
    ...
    public static function create()
    {
        try {
            ...
            if (
                empty($request->date_debut) || empty($request->date_fin) ||
...
public static function getMontantTotalReste()
{
    // Ajoutez ces logs pour débugger
    error_log("Paramètres reçus: " . print_r(Flight::request()->query, true));
    ...
    try {
        $entrees = PretModel::getSommeEntrees($moisDebut, $anneeDebut, $moisFin, $anneeFin);
        $sorties = PretModel::getSommeSorties($moisDebut, $anneeDebut, $moisFin, $anneeFin);
        ...
        error_log("Erreur dans getMontantTotalReste: " . $e->getMessage());
        Flight::json([
            'error' => 'Erreur lors du calcul du montant restant: ' . $e->getMessage()
        ], 500);
    }
}
    ...
    public static function generatePDF()
    {
        ...
        // Validation des données
        if (
            empty($nom) || empty($typePret) || empty($duree) || empty($montantEmprunter) ||
